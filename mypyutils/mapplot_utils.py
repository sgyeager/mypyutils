...
def map_contourf_global_subplot(fig, dat, lon, lat, ci, cmin, cmax, titlestr, 
			        leftstr, rightstr, nrow,ncol,subplot, proj, labels=True, showland=True, 
				extend='neither', grid="latlon", cmap="blue2red", facecolor="white",
				fontsize=15, centrallon=0,tricontour=False):
    """ plot a contour map of 2D data dat with coordinates lon and lat
        Input:
              fig = the figure identifier
              dat = the data to be plotted
              ...
              cmax = the maximum of the contour range
              titlestr = the title of the map
              leftstr = top left string
              rightstr = top right string
              ...
              extend = neither/both/max/min (extend color bar)
              grid = ('latlon','camfv','camse','pop')
              cmap = color map (only set up for blue2red at the moment)
              facecolor = background color on contour map
    """
    ...
    if grid=="latlon" or grid=="camfv":
        ...
        cntr = ax.tricontourf(lon, lat, dat,
                              levels=clevs,
                              cmap = mymap, extend=extend, transform=ccrs.PlateCarree())
