...
def plotcolorbar(fig, ci, cmin, cmax, titlestr, x1, x2, y1, y2,
   cmap='blue2red', orient='horizontal', posneg='both', extend='both', ticks=None, fsize=14):
    """plot a color bar
       Input:
           ...
           extend = if "both", both positive and negative sides are extended
                    if "neither", neither side is extended
                    if "min", only negative side is extended
                    if "max", only positive side is extended
